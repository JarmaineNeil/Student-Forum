...
<?php
if (isset($_GET['id'])) {
    $stmtTopic = $conn->prepare("SELECT t.*, u.name FROM topics t INNER JOIN users u ON u.id = t.user_id WHERE t.id = :id");
    $stmtTopic->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
    $stmtTopic->execute();

    $resultTopic = $stmtTopic->fetch(PDO::FETCH_ASSOC);

    foreach ($resultTopic as $k => $val) {
        $$k = $val;
    }

    $stmtComments = $conn->prepare("SELECT c.*, u.name, u.username FROM comments c INNER JOIN users u ON u.id = c.user_id WHERE c.topic_id = :id ORDER BY UNIX_TIMESTAMP(c.date_created) ASC");
    $stmtComments->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
    $stmtComments->execute();

    $com_arr = $stmtComments->fetchAll(PDO::FETCH_ASSOC);

    $stmtReplies = $conn->prepare("SELECT r.*, u.name, u.username FROM replies r INNER JOIN users u ON u.id = r.user_id WHERE r.comment_id IN (SELECT id FROM comments WHERE topic_id = :id) ORDER BY UNIX_TIMESTAMP(r.date_created) ASC");
    $stmtReplies->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
    $stmtReplies->execute();

    $rep_arr = [];
    while ($row = $stmtReplies->fetch(PDO::FETCH_ASSOC)) {
        $rep_arr[$row['comment_id']][] = $row;
    }

    if ($user_id != $_SESSION['login_id']) {
        $stmtCheckView = $conn->prepare("SELECT * FROM forum_views WHERE topic_id = :id AND user_id = :user_id");
        $stmtCheckView->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
        $stmtCheckView->bindParam(':user_id', $_SESSION['login_id'], PDO::PARAM_INT);
        $stmtCheckView->execute();

        $chk = $stmtCheckView->rowCount();

        if ($chk <= 0) {
            $stmtInsertView = $conn->prepare("INSERT INTO forum_views SET topic_id = :id, user_id = :user_id");
            $stmtInsertView->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
            $stmtInsertView->bindParam(':user_id', $_SESSION['login_id'], PDO::PARAM_INT);
            $stmtInsertView->execute();
        }
    }

    $stmtViewCount = $conn->prepare("SELECT * FROM forum_views WHERE topic_id = :id");
    $stmtViewCount->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
    $stmtViewCount->execute();

    $view = $stmtViewCount->rowCount();

    $tags = [];
    if (!empty($category_ids)) {
        $stmtTag = $conn->prepare("SELECT * FROM categories WHERE id IN ($category_ids) ORDER BY name ASC");
        $stmtTag->execute();

        while ($row = $stmtTag->fetch(PDO::FETCH_ASSOC)) {
