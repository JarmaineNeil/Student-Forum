				...
				<div class="card">
					...
					<div class="card-body">
						<ul class="w-100 list-group" id="topic-list">
							...
							$topicStmt = $conn->query("SELECT t.*, u.name FROM topics t INNER JOIN users u ON u.id = t.user_id ORDER BY UNIX_TIMESTAMP(t.date_created) DESC");
							...
							while ($row = $topicStmt->fetch(PDO::FETCH_ASSOC)) {
								...
								$viewStmt = $conn->prepare("SELECT * FROM forum_views WHERE topic_id = :topic_id");
								$viewStmt->bindParam(':topic_id', $row['id'], PDO::PARAM_INT);
								$viewStmt->execute();
								$view = $viewStmt->rowCount();

								$commentsStmt = $conn->prepare("SELECT * FROM comments WHERE topic_id = :topic_id");
								$commentsStmt->bindParam(':topic_id', $row['id'], PDO::PARAM_INT);
								$commentsStmt->execute();
								$comments = $commentsStmt->rowCount();

								$repliesStmt = $conn->prepare("SELECT * FROM replies WHERE comment_id IN (SELECT id FROM comments WHERE topic_id = :topic_id)");
								$repliesStmt->bindParam(':topic_id', $row['id'], PDO::PARAM_INT);
								$repliesStmt->execute();
